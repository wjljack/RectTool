...
namespace CSharpWin_JD.CaptureImage
{
    ...
    public partial class DrawToolsControl : UserControl
    {
        ...
        public void SetAcceptButtonVisible(bool visible)
        {
            toolStripButtonAccept.Visible = visible;
        }
        public void SetAcceptButtonEnable(bool enable)
        {
            toolStripButtonAccept.Enabled = enable;
            if (enable)
            {
                toolStripButtonAccept.ToolTipText = "完成设置";
            }
            else
            {
                toolStripButtonAccept.ToolTipText = "与已有房间重叠，请重新选择位置";
            }
        }
